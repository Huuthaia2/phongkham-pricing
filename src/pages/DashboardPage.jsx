    ...
    <div className="space-y-4 animate-slideUp">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <div className="font-secondary uppercase tracking-wider font-bold text-slate-700 text-sm">Báo giá gần nhất</div>
          <button onClick={()=>setActiveTab('quotes')} className="text-indigo-600 text-xs font-secondary uppercase tracking-wider font-bold hover:underline">Xem tất cả →</button>
        </div>
        ...
        {(data?.recentQuotes||[]).slice(0,6).map(q => (
          <div key={q.MaBaoGia} className="px-4 py-3 flex items-center gap-3 border-b border-slate-50 last:border-0 hover:bg-slate-50">
            ...
            <div className="text-right flex-shrink-0">
              <div className="font-secondary font-bold text-base text-indigo-600">{fmtM(q.GiaToiUu)}</div>
