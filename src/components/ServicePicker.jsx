...
export default function ServicePicker() {
  ...
  if (loading) return <div className="flex items-center justify-center h-32 text-slate-400 text-sm">⚙️ Đang tải dịch vụ...</div>
  if (error)   return <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-red-700 text-sm">⚠️ {error}</div>
            ...
            <div key={svc.MaDichVu}
              ...
              <div className="p-3.5 flex items-start gap-3 cursor-pointer select-none active:bg-slate-50 transition-colors"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {svc.GhiChu && <div className="text-xs text-amber-600 mt-0.5">⚠️ {svc.GhiChu}</div>}
                  {svc.QuaTang && <div className="text-xs text-green-600 mt-0.5">🎁 {svc.QuaTang}</div>}
